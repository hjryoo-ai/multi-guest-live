...
- **E2E 대기 시간은 대응하는 제품 폴백 주기의 2배를 초과해야 한다.** 폴백 폴링은 "폴이 정시에 돌고
  refresh 가 성공하면 1주기 안에 뜬다"를 보증하지만, **부하에서는 폴 1주기가 유실될 수 있다** —
  요청이 폴 N 직후 들어오고(≈+1주기) 폴 N+1 의 refresh 가 부하로 유실/지연되면(≈+1주기) 보증까지
  최대 2주기가 걸린다. 따라서 하한은 "1주기 초과"가 아니라 **"1주기 유실을 견딤(≥2×)"** 이다.
  (PR#12 는 이를 1.5× 로 잘못 잡아 @heavy `phase6a` 후반 speaker(S4/S3)에서 재발 — 항상 深부하
  지점, 2회 연속 실패로 확인.)
- **파생으로 강제 + 머신 체크**(매직넘버·프로즈 금지): 큐행 대기 = `QUEUE_POLL_MS × 2.5`
  (`e2e/helpers.QUEUE_ROW_WAIT_MS`; 2주기 40s + refresh 여유 10s). `helpers.ts` 가 로드 시점에
  `QUEUE_ROW_WAIT_MS ≥ 2 × QUEUE_POLL_MS` 를 **단언**한다(위반 시 스펙 로드 실패) — 프로즈 주석
  두 곳이 PR#12 의 잘못된 배수를 막지 못했으므로 하한을 코드로 못박는다. 폴링 주기를 바꾸면
  대기도 자동으로 함께 움직인다.
