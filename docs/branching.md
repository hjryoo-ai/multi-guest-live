...
- **게이트 정의(수치 정본)**: `gate` = typecheck(전체) + verify(1~6.5 + §7-lite 프록시신뢰·데모가드·LiveKit타임아웃) + **Playwright E2E 15개** = 핵심 11 + 모바일 스모크 4종(390×844: 시청·게스트·호스트 + **데모**). `@heavy`(8명 전수·egress·reconnect)와 `screenshots.spec`은 게이트 제외(→ `nightly.yml`). **이 15가 수치 정본**이며 ci.yml 스텝 라벨·PR 본문 수치는 여기에 맞춘다(라벨 드리프트 방지).
...
1. **변경**: 무엇을 왜 (설계문서 § 참조)
2. **검증**: 통과한 verify/E2E 스코프 (예: "E2E 15/15, verify 1~6.5 + §7-lite 그린")
...
| 6 | `feat/7lite-demo-ux` | §1-5 데모 시작·QR·가이드 (타임아웃은 5b 로 분리) | ← **현재 PR** · testid 신규·게이트 14→15 동반 |
| 6b | `chore/ci-actions-bump` | `actions/*@v4` Node20 deprecation 경고 해소(동작 변경 0) | cd-wire 직전(CD 부활 전 CI에서 격리 범프) |
